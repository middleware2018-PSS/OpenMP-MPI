...
int main() {
    int K = 0;
    int T = 3000000000000; //30 seconds
    string path = "../";
    ...
    for (auto record : g.records)
        for (auto sensor : record) {
            ...
            if (g.activate_offset)
                timestamp_offset = sensor.game_timestamp;
            ...
            if (first_half_starting_time <= sensor.game_timestamp <= second_half_ending_time && g.is_interrupted(sensor.game_timestamp) && g.is_inside_field(sensor))
                //cout << "ok" << endl;
            //check if the sensor is a player's sensor (or add ! for a ball)
                if (!g.is_player_sensor_id(sensor.sensor_id)) {
                    //cout << "palla!" << endl;
                    cout << sensor.game_timestamp << " " << sensor.sensor_id << " " << sensor.x << " " << sensor.y << " "
                         << sensor.z << endl;
                        ...
                        for (auto position : g.lastPos){
                            ...
                            if (temp < minimum_distance) {
                                ...
                                if (g.nearest_sensor == position.first)
                                    int delta = sensor.game_timestamp - tempdelta;
                                g.nearest_sensor = position.first;
                                tempdelta = sensor.game_timestamp;
